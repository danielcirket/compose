...
namespace Compose.Tests
{
	public class TransitionTests
	{
		...
		internal sealed class TertiaryProvider : Service { }

		[Unit]
		public static void WhenAddingProviderThenReturnsProviderAbstraction()
		{
			var provider = new Fake.Application()
				.AddProvider<Service>(services => services.AddTransient<TertiaryProvider>());
			Assert.NotNull(provider);
		}

		[Unit]
		public static void WhenResolvingFromProviderWithoutImplementationOfServiceThenThrowsException()
		{
			Action act = () => new Fake.Application().AddProvider<Service>(services => services.AddTransient<TertiaryProvider>()).GetService();
			Assert.Throws<InvalidProviderException>(act);
		}

		[Unit]
		public static void WhenProviderTransitionedToThenCanResolveNewProvider()
		{
			var app = new Fake.Application();
			app.Transition(app.AddProvider<Service>(services => services.AddTransient<TertiaryProvider>()));
			Assert.NotNull(app.ApplicationServices.GetService<Service>());
		}
